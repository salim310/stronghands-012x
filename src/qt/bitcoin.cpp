 ...
 * The StrongHands Developers 2013
 */
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occured. StrongHands can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
// TODO: implement qtipcserver.cpp for Mac and Windows
    ...
    {
        if (strlen(argv[i]) >= 7 && strncasecmp(argv[i], "stronghands:", 7) == 0)
        {
  ...
  // Basic Qt initialization (not dependent on parameters or configuration)
    ...
    app.setOrganizationName("StrongHands");
    app.setOrganizationDomain("stronghands.org");
    if(GetBoolArg("-testnet")) // Separate UI settings for testnet
        app.setApplicationName("StrongHands-Qt-testnet");
    else
        app.setApplicationName("StrongHands-Qt");
